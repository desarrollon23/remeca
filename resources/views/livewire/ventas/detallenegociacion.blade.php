      ...
      <div class="container-fluid mx-auto">
        <div class="row aling: center">
          <div class="col-lg-12 col-md-12 col-xs-12 mt-2">
            ...
            if($negociaciones->count()>0 and $creditos->count()>0){
              $sumamonto=(double)$negociaciones->sum('montototal')+(double)$creditos->sum('montototal');
              $sumapagado=(double)$negociaciones->sum('totalpagado')+(double)$creditos->sum('totalpagado');
              $sumaresta=(double)$negociaciones->sum('totalresta')+(double)$creditos->sum('totalresta');
            }elseif($negociaciones->count()==0 and $creditos->count()>0){
              $sumamonto=(double)$creditos->sum('montototal');
              $sumapagado=(double)$creditos->sum('totalpagado');
              $sumaresta=(double)$creditos->sum('totalresta');
            }elseif($negociaciones->count()>0 and $creditos->count()==0){
              $sumamonto=(double)$negociaciones->sum('montototal');
              $sumapagado=(double)$negociaciones->sum('totalpagado');
              $sumaresta=(double)$negociaciones->sum('totalresta');
            }
                  @endphp
                  ...
                  <div class="card-body" style="display: flex; flex-wrap: wrap; margin-right: 2px;">
                    @foreach ($datosnegociaciones as $negociacion)
                    <div class="col-lg-12 col-md-12 col-xs-12 mt-2">
                      <div class="card">
                        <div class="card-header bg-info">
                          <div style="display: flex; flex-wrap: wrap; margin-right: 2px;">
                      ...
                      <div class="card">
                        <div class="card-header bg-info">
                          ...
                          <table class="table table-striped"><thead><tr>
                              ...
                              @foreach ($productosrecepcioncreditos as $productorecepcion)
                                @if ($credito->idventa==$productorecepcion->idventa)
                                @php $cantidades[$loop->iteration]=$productorecepcion->cantidadprov  @endphp
                                <tr><td scope="row">{{ $productorecepcion->descripcion }}</td>
                                  <td>{{ $formatter->formatCurrency($productorecepcion->cantidadprov, ''), PHP_EOL }}</td>
                                  <td><a><i class="color danger fas fa-asterisk text-success"></i></a></td>
                                  <td>{{ $formatter->formatCurrency($productorecepcion->precioprov, ''), PHP_EOL }}</td>
                                  <td><a><i class="color danger fas fa-equals text-success"></i></a></td>
                                  <td>{{ $formatter->formatCurrency($productorecepcion->totalprov, ''), PHP_EOL }}
                                  </td>
                                  </td>
                                </tr>
                                @endif
                            ...
                            <tfoot><tr>
                              <td><label>TOTAL</label></td>
                              <td></td><td></td><td></td><td></td>
                              <td><h1 style="font-weight:900; color:red">{{ $formatter->formatCurrency($credito->montototal, ''), PHP_EOL }}</h1>
                              </td></tr>
                            </tfoot>
                          </table>
