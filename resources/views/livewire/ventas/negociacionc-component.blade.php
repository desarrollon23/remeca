              ...
              <div class="col-lg-4 col-md-6 col-xs-6 mt-2">
                <div class="card">
                  ...
                  <div class="card-body" style="display: flex; flex-wrap: wrap; margin-right: 2px;">
                    <div style="width: 39%; margin-right: 2px;"><label>Cédula o Rif
                      <input x-bind:disabled="!{{ $mostrar }}" wire:model="cedulan" wire:keyup="busproc" style="width: 100%" id="cedulan" class="form-control" type="text" name="cedulan" placeholder="INGRESE CEDULA O RIF">
                      @error('cedulan')<p class="text-x text-red-500 italic">{{$message}}</p>@enderror
                    </label></div>
                    <div style="width: 59%; margin-right: 2px;"><label style="width: 100%; margin-right: 2px;">Nombre
                ...
                <div class="card">
                        ...
                        <div style="width: 100%; display: flex; flex-wrap: wrap; margin-right: 2px;">
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                            ...
                            <select x-show="{{ $mostrar }}" x-bind:disabled="!{{ $mostrar }}" name="producto_idn" wire:model="producto_idn" id="producto_idn" class="form-control">
                              ...
                              @endforeach
                             </select><p x-show="{{ $mostraremid }}" class="text-x text-red-500 italic">SELECCIONE</p></label>
                          </div>
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                            <label for="cantidadprorecmatn" style="width: 100%; margin-right: 2px;">Peso
                            <input x-show="{{ $mostrar }}" x-bind:disabled="!{{ $mostrar }}" type="number" wire:model="cantidadprorecmatn" class="form-control" id="cantidadprorecmatn" name="cantidadprorecmatn" aria-describedby="cantidadprorecmatn" placeholder="Ingrese la cantidad" wire:keyup="calpeso"><p x-show="{{ $mostraremc }}" class="text-x text-red-500 italic">INGRESE</p>
                            <small id="emailHelp" class="form-text text-muted"></small></label>
                          </div>
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                            <label for="precionegn" style="width: 100%; margin-right: 2px;">Precio
                            <input x-show="{{ $mostrar }}" x-bind:disabled="!{{ $mostrar }}" type="number" wire:model="precionegn" class="form-control" id="precionegn" name="precionegn" aria-describedby="precionegn" placeholder="Ingrese la cantidad" wire:keyup="calpeso"><p x-show="{{ $mostraremp }}" class="text-x text-red-500 italic">INGRESE</p>
                            <small id="emailHelp" class="form-text text-muted"></small></label>
                          </div>
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                  ...
                  <table class="table table-striped"><thead><tr> 
                    ...
                    <tfoot><tr><td colspan="2"><label>TOTAL</label></td>
                      <td><h1 style="font-weight:900; color:red">{{ (double)$pesotn }}
                      </h1></td><td></td><td></td><td></td>
                      <td><h1 style="font-weight:900; color:red">{{ (double)$montotn }}</h1>
                    </td><td></td></tr>
                      <tr><td></td></tr>
                    </tfoot>
                  </table>
