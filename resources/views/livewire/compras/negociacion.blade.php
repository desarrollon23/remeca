...
<div class="pt-1">
    ...
    <div x-data="main()" class="content">
      <div class="container-fluid mt-2 padding-2">
                ...
                <div class="card">
                  ...
                  <div class="card-body" style="/* background-color: #9FA5AA; */ display: flex; flex-wrap: wrap; margin-right: 2px;">
                    <div style="width: 39%; margin-right: 2px;"><label>Cédula o Rif</label>
                      <input x-bind:disabled="!{{ $mostrard }}" wire:model="cedulan" wire:keyup="busproc" style="width: 100%" id="cedulan" type="text" name="cedulan" placeholder="INGRESE CEDULA O RIF">
                    ...
                    <div style="width: 59%; margin-right: 2px;"><label style="width: 100%; margin-right: 2px;">Nombre</label>
                      <input x-bind:disabled="!{{ $mostrard }}" wire:model="nombren" wire:keyup="buspron" style="width: 100%; text-transform: uppercase;" id="nombren" type="text" name="nombren" placeholder="Ingrese el Nombre">
                    ...
                    <div style="width: 100%; display: flex; flex-wrap: wrap;">
                      ...
                      <div x-show="{{ $nopagar }}" style="width: 49%; margin-right: 2px;">
        ...
        @if ($vendedores[0]->where('cedula', $cedulan)->count())
          @php 
            if($cedulan!=session('valorcedulannc')){
                $ocultarboton="false"; $this->cantidadprorecmatn=''; $this->precionegn='';
                /* $this->validamontotn=""; */ $this->totalpronegn=0; /* $this->restapagoneg=0; */
                session(['valorcedulannc' => $cedulan]);
            } 
          @endphp
                ...
                <div class="card">
                  ...
                  <div class="card-body" style="/* background-color: #9FA5AA; */ display: flex; flex-wrap: wrap; margin-right: 2px;">
                        <div style="width: 100%; display: flex; flex-wrap: wrap; margin-right: 2px;">
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                            ...
                            <select x-show="{{ $mostrar }}" x-bind:disabled="!{{ $mostrar }}" name="producto_idn" wire:model="producto_idn" id="producto_idn" wire:change="traerprecio" style="width: 100%;">
                          ...
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                            ...
                            <input x-show="{{ $mostrar }}" {{-- x-bind:disabled="!{{ $mostrar }}" --}} type="text" wire:model="precionegn" id="precionegn" name="precionegn" aria-describedby="precionegn" placeholder="Ingrese la cantidad" wire:keyup="calpeso" size="10" maxlength="10" min="0" max="9999999999" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" onkeypress="mascara(this,cpf)" onpaste="return false" style="width: 100%; text-align: right;" disabled><p x-show="{{ $mostraremp }}" class="text-x text-red-500 italic">INGRESE</p>
                            <small id="emailHelp" class="form-text text-muted"></small>
                          </div>
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                            <label for="totalpronegn" style="width: 100%; margin-right: 2px;">Total</label>
                            <input x-show="{{ $mostrar }}" type="text" id="totalpronegn" name="totalpronegn" wire:model="totalpronegn" placeholder="TOTAL" style="width: 100%; text-align: right;" disabled>
                          ...
                          <div x-show="{{ $mostrarm }}" style="width: 19%; margin-right: 2px;">
                            <label for="" style="width: 100%; margin-right: 2px;" class="align-self-baseline">
                              <button x-show="{{ $ocultarboton }}" wire:click="storem" class="btn btn-primary mt-3"><i class="fa fa-save mr-1"></i>Agregar</button>
                            </label>
                          </div>
                        </div>
                  ...
                  <table class="table table-striped" {{-- style="background-color: #9FA5AA;" --}}><thead><tr> 
                    ...
                    <tr><th scope="row">{{ $loop->iteration }}</th>
                      ...
                      <td>{{ $formatter->formatCurrency($productorecepcion->precionegn, ''), PHP_EOL }}</td>{{ $formatter->formatCurrency(round($bancodis,2), ''), PHP_EOL }}
                    ...
                    <tfoot><tr><td colspan="3"><label>TOTAL A PAGAR</label></td>
                      ...
                      <tr><td><label>FACTURACION</label></td>
                        ...
                        <td><label style="width: 100%; margin-right: 2px;">Efectivo <label style="color: red;">{{ $formatter->formatCurrency(round($efectivodis,2), ''), PHP_EOL }}</label></label><span class="parpadea pago">{{ $validamontotne }}</span></td><td></td>
                        <td><label style="width: 100%; margin-right: 2px;">Transferencia <label style="color: red;">{{ $formatter->formatCurrency(round($bancodis,2), ''), PHP_EOL }}</label></label><span class="parpadea pago">{{ $validamontotnt }}</span></td><td></td><td></td>
                      </tr>
                      ...
                      <tr><td><label></label></td>
                        ...
                        <td>
                          <input x-bind:disabled="!{{ $mostrar }}" type="number" wire:model="pagoefectivoneg" id="pagoefectivoneg" name="pagoefectivoneg" aria-describedby="pagoefectivoneg" placeholder="Efectivo" wire:keyup="calrestaneg({{$montotn}})" size="10" maxlength="10" min="0" max="9999999999" onkeypress="mascara(this,cpf)" onpaste="return false" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" style="width: 100%;">
                        </td>
                        <td><a><i class="color danger fas fa-plus text-success"></i></a></td>
                        <td>
                          <input x-bind:disabled="!{{ $mostrar }}" type="number" wire:model="pagotransfneg" id="pagotransfneg" name="pagotransfneg" aria-describedby="pagotransfneg" placeholder="Transferencia" wire:keyup="calrestaneg({{$montotn}})" size="10" maxlength="10" min="0" max="9999999999"onkeypress="mascara(this,cpf)" onpaste="return false" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" style="width: 100%;">
                        </td>
                        <td><a><i class="color danger fas fa-equals text-success"></i></a></td>
